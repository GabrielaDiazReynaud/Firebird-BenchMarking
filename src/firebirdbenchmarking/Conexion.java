...
public class Conexion {
    ...
    public void llenarTabla(ArrayList<String> tipos, ArrayList<Integer> lengthfields, String namet) {
        String query = " INSERT INTO " + namet + " VALUES (";
        for (int x = 0; x < tipos.size(); x++) {
            if (tipos.get(x).equals("INTEGER")) {
                Random r = new Random();
                int num = r.nextInt(1000000);
                query += " " + num + " ";
            } else if (tipos.get(x).equals("CHAR")) {
                Random r = new Random();
                String abc = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
                char letter = abc.charAt(r.nextInt(abc.length()));
                query += " '" + letter + "' ";
            } else if (tipos.get(x).equals("DATE")) {
                LocalDate randomDate = createRandomDate(1900, 2000);
                query += " '" + randomDate + "' ";

            } else if (tipos.get(x).equals("DECIMAL") || tipos.get(x).equals("DOUBLE")) {
                Random rd = new Random();
                double db = nextDoubleBetween2(0.0d, 100.0d);
                query += " " + db + " ";
            } else if (tipos.get(x).equals("FLOAT")) {
                Random rd = new Random();
                float fl = nextFloatBetween2(0.0f, 100.0f);
                query += " " + fl + " ";
            } else if (tipos.get(x).equals("VARCHAR")) {
                int n = 0;
                Random r = new Random();
                String abc = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
                String word = "";
                while (n < lengthfields.get(x)) {
                    System.out.println(lengthfields.get(x));
                    char letter = abc.charAt(r.nextInt(abc.length()));
                    word += letter;
                    n++;
                }
    ...
    public ArrayList<Integer> fieldlength(String name) {
        ArrayList<Integer> length = new ArrayList<Integer>();
        try {
            String query = "SELECT c.RDB$FIELD_LENGTH FROM   RDB$RELATIONS a INNER JOIN RDB$RELATION_FIELDS b ON a.RDB$RELATION_NAME = b.RDB$RELATION_NAME INNER JOIN RDB$FIELDS c ON b.RDB$FIELD_SOURCE = c.RDB$FIELD_NAME INNER JOIN RDB$TYPES d ON c.RDB$FIELD_TYPE = d.RDB$TYPE WHERE  a.RDB$SYSTEM_FLAG = 0 AND  d.RDB$FIELD_NAME = 'RDB$FIELD_TYPE' AND b.RDB$RELATION_NAME= '" + name + "' ;";
            Statement st;
            st = con.createStatement();
            ResultSet rs = st.executeQuery(query);
            while (rs.next()) {
                length.add(rs.getInt(1));
    ...
    public ArrayList<String> types(String name) {
        ...
        try {
            ...
            ArrayList<String> a = TablesColumnandType(name);
            for (int x = 1; x <= columnas; x++) {
